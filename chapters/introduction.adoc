...
Nous allons aborder au <<methode>> les processus connus et courants en Ingénierie Système basé sur les Modèles généralement associés à {SysML}.
...
Comme le résume la <<piliers,figure précédente>>, nous allons aborder dans la pratique aussi bien les éléments de notation ({sysml}), de méthodologie et d'outillage ({papyrus}).
...
image::dessins/standards.svg[width="60%",scaledwidth="60%"]

Nous utiliserons dans ce livre une démarche minimaliste préconisée par <<Friedenthal2016>>.
Cette méthode ainsi que les références aux démarches principales seront abordées au <<methode>>.
...
Sans couvrir l'ensemble des activités d'ingénierie système (ce qui nécessiterait tout un volume du type de <<Sommerville1997>>) nous insisterons sur cet aspect.
...
{Crosscutting}::
Un certain nombre de concepts sont transverses aux trois points de vue précédents.
...
Dans un axe vertical, nous différencions quatre grandes phases du cycle de vie du développement :
...
Après la définition de quelques éléments méthodologiques (chapitre <<methode>>), nous attaquons la manipulation proprement dite de {papyrus} (chapitre <<start>>).
Nous rentrons dans le détail de {sysml} avec les chapitres suivants :
en commençant par les exigences (chapitre <<reqs>>), nous poursuivons par l'utilisation du système (cas d'utilisation et interfaces, chapitre <<Usages>>), puis traitons des éléments structurels (chapitre <<archi>>) et
comportementaux (chapitre <<behavior>>).
Puis des interaction (chapitre <<interaction>>) et des préoccupations transverses (chapitre <<crossCuting>>).

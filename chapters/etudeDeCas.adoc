...
. Cette liste d'exigences sert de base pour étudier la partie d'analyse (_elicitation_ en anglais) des exigences.
Elles ne sont donc pas parfaites mais sont le reflet de la réalité.
. Les exemples de modèles réalisés, pour des soucis de partage plus large avec la communauté, sont
réalisés en anglais à partir de la version anglaise des exigences (disponible {siteUseCase}[ici]).
=====
...
personnes à domicile qui sont autonomes mais qui doivent être suivies ou aidées.
...
Comme n'importe quel autre logement, la maison intelligente doit être sécurisée.

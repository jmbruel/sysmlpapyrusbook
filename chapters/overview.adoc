...
(`{uc}` pour "diagramme des UC" par exemple). Ces abréviations, sont définies dans la spécification.
...
Mis à part les deux diagrammes <<reqs, des exigences>> (`{req}`, représentation visuelle des exigences) et <<package, de _packages_>> (`{pkg}`, représentation visuelle des _packages_), les autres diagrammes
...
Le <<param,diagramme paramétrique>> (`{par}`) n'est qu'un diagramme de blocs internes particulier, dédié aux relations entre valeurs
...
| {organisation} | {pkg} | {pkg} | {pkg}, {bdd}, {ibd} | {pkg} | {pkg} .5+<.>| {par}, {pkg}
| {context} 5+| {pkg}, {bdd}, {ibd}
| {analysis} .3+<.>| {req}, {uc}, {seq} .3+<.>| {stm} .3+<.>|  {bdd}, {ibd}, {par} .3+<.>|  {uc}, {seq}, {stm}, {act} .3+<.>|  {ibd}
| {design}
| {implementation}	|
|======================
...
image::{diagrams}/SHS_Use_Cases_Description.PNG[width=100%,scaledwidth=70%]

...
Un ensemble de 9 types de diagrammes (cf. <<diags>>)::
...
Il est intéressant de se rappeler que {sysml} est un standard qui vient des utilisateurs,
...
Il va de soit que le meilleur outil {sysml} à ce jour est {papyrus}footnote:[La partialité des auteurs peut être questionnée, mais d'un point de vue complétude et respect de la norme, cette affirmation est régulièrement admise par la communauté.], dont ce livre vous détaille l'utilisation, mais il existe un certain nombre d'autres outils
...
Les ingénieurs systèmes ont l'habitude d'utiliser des représentations graphiques (plans,
	graphes, ...).
...
La figure suivante présente cette organisation en faisant, au passage, le lien avec ceux d'{uml} :
...
Si l'on met à part le <<reqs,diagramme des exigences>> (représentation visuelle des exigences) et
le <<package,diagramme de _packages_>> (représentation visuelle des _packages_), les autres diagrammes
nécessitent de préciser l'utilisation que nous préconisons d'en faire.

Pour les diagrammes structuraux, quand il s'agira de représenter les architectures,
nous utiliserons les <<bddsec,diagrammes de définition de blocs>>, et pour représenter l'intérieur d'un système,
nous utiliserons le <<ibd,diagramme de bloc interne>>.
Le <<param,diagramme paramétrique>> n'est qu'un diagramme de bloc interne dédié aux relations entre valeurs
(pour représenter des lois physiques par exemple).

Pour les diagrammes comportementaux, nous utilisons la distinction entre comportement
global et local, comme indiqué dans la <<comportements,figure suivante>>.
Ainsi les <<usecase,diagramme des cas d'utilisation>> et les <<seq,diagrammes de séquences>>
seront plutôt utilisés pour représenter le coprtement global tel qu'attendu par les partie prenantes
du système.
Tandis que les <<act,diagrammes d'activité>> et les <<stm,diagrammes d'état>> seront plutôt
...
La gestion des exigences est une activité cruciale pour le développement correct de tout système.
Il s'agit généralement de la première étape d'un développement : lister les besoins du client
pour s'assurer qu'on les a bien compris et pour pouvoir lier la suite de la modélisation et
du développement à des besoins bien particuliers.
...
Il sera vu plus en détail dans la section <<reqs>>, mais en voici un exemple.
...
Les exigences sont plus souvent manipulées et représentées sous une forme tabulaire comme illustré dans la figure suivante :

.Exemple de table d'exigence pour le {projectName}
image::{papyrusversion}/table.png[width=70%,scaledwidth=70%]

En effet les principaux logiciels de gestion des exigences sont basés sur cette représentation tabulaire.

TIP: Ce format tabulaire permet d'ailleur d'importer/exporter facilement des exigences dans {papyrus}
(cf. [red]#XXX ??? XXX#).

== Diagramme des cas d'utilisation

Une autre façon de modéliser les besoins utilisteurs, plus orientée {uml}, consiste
à représenter les acteurs (appelés _Actors_) qui manipulent le système en les reliant aux différentes
utilisations (appelées _Use Cases_) possibles du système.

.Exemple de diagramme de cas d'utilisation
image::{diagrams}/SHS_Use_Cases_Description.PNG[width=70%,scaledwidth=70%]

Ce diagramme sera vu plus en détail dans la section <<uc-concepts>>.

== Diagramme de blocs (BDD)

Les modèles d'un système se décomposent en deux grandes familles :

- les modèles comportementaux,
qui servent à représenter la dynamique du système, son comportement;
- les modèles structuraux,
qui servent à représenter l'organisation du système en sous-systèmes et
ainsi de suite jusqu'à arriver à des composants élémentaires.

Le diagramme de blocs (_Blocks Definition Diagram_) est un diagramme structurel
qui permet de représenter les liens entre blocs du système.
On parle parfois de "vue boîte noire" car on ne représente que les composants d'un
système et non leur organisation dans ce système.
...
Ce diagramme sera vu plus en détail dans la section <<bddsec>>.

== Diagramme de bloc interne (IBD)

Le deuxième diagramme structurel de {sysml}, peut-être encore plus utilisé que le diagramme de blocs,
est le diagramme de bloc interne (_Internal Blocks Diagram_)
On parle parfois de "vue boîte blanche" car on représente non seulement les
composants d'un système mais aussi leur organisation dans ce système.

.Exemple de diagramme de bloc interne
image::todo.jpg[]
...
=====
// Sebastien
Nous avons parlé du diagramme de contexte dans la section précédente sur le diagramme de blocs.
Le diagramme de bloc interne est souvent utilisé pour représenter le contexte d'un système
car il est plus précis (il décrit notamment comment sont reliés les éléments constitutifs).
...
Ce diagramme sera vu plus en détail dans la section <<ibd>>.
...
Le diagramme de séquence (_Sequence Diagram_) est un diagramme populaire pour représenter les
...
Dans ce diagramme, appelé diagramme de séquence système (<<dss,DSS>>), le seul
...
Les diagrammes de séquence système (DSS) sont des _Sequence Diagrams_ {uml}
classiques où seul le système est représenté comme une boîte noire en
interaction avec son environnement (les utilisateurs généralement).

Il permet de décrire les scénarios des cas d'utilisation sans entrer dans les détails.
Il convient donc mieux à l'ingénierie système qu'un diagramme de séquence
classique.
...
Le diagramme de séquence permet de représenter les différents participants
...
Le diagramme de séquence sera vu plus en détail dans la section <<seq>>.
...
NOTE: Un _package_ n'est pas un composant mais une unité d'organisation du modèle.
